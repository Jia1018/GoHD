...
from networks.core.self_attention_pooling import SelfAttentionPooling
...
class StyleEncoder(nn.Module):
    ...
    def forward(self, x, cond=None, pad_mask=None):
        """
...
class CrossAttGenerator(nn.Module):
    def __init__(
        self,
        d_model=512,
        nhead=8,
        ...
        **_,
        ...
        decoder_layer = DynamicFCDecoderLayer(d_model, nhead, dim_feedforward, dropout, activation, normalize_before)
        decoder_norm = nn.LayerNorm(d_model)
        self.decoder = DynamicFCDecoder(
            decoder_layer,
            num_decoder_layers,
            decoder_norm,
            return_intermediate=return_intermediate_dec,
    ...
    def forward(self, content, style):
        ...
        Args:
            content (_type_): (B, num_frames, C_3dmm)
            style (_type_): (B, num_frames, C_3dmm)
        ...
        # (B, C)
        style = style_code.unsqueeze(0).repeat(T, 1, 1)
