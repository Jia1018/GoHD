...
from networks.generators import flow_util
from networks.generators.base_function import LayerNorm2d, ADAINHourglass, FineEncoder, FineDecoder
...
class EditingNet(nn.Module):
    def __init__(
        ...
        if gaze:
            self.gaze_encoder = MappingNet(2, descriptor_nc, 3)
    ...
    def forward(self, input_image, warp_image, descriptor, gaze=None):
        x = torch.cat([warp_image, input_image], 1)
        x = self.encoder(x)
        if gaze is not None:
            gaze = self.gaze_encoder(gaze)
            descriptor = torch.cat((descriptor, gaze), dim=-1)
