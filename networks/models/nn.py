...
from networks.models.transformer.tisa_transformer import TisaTransformer
...
class ResidualBlock(nn.Module):
  def __init__(self, residual_channels, 
    ...
    if nn_name=="tisa":
        ...
        self.nn = TisaTransformer(residual_channels, 2 * residual_channels, d_model=residual_channels, num_blocks=2, num_heads=8, activation='relu', norm='LN', drop_prob=0.1, d_ff=1024, seqlen=params.pose_seq_len, use_preln='false', bias='false', dilation=dilation)
    elif nn_name=="conv":
        dilation=2**(index % params.dilation_cycle_length)
...
class LDA(nn.Module):
  def __init__(self, params, nn_name):
    super().__init__()
    self.input_projection = Conv1dLayer(params.in_channels, params.residual_channels, 1)
    self.diffusion_embedding = DiffusionEmbedding(params.n_noise_schedule, 128, params.embedding_dim)

    self.residual_layers = nn.ModuleList([
        ResidualBlock(params.residual_channels,
            params.embedding_dim,
            params.n_mels + params.style_dim,
            nn_name,
            params,
            i)
        for i in range(params.residual_layers)
    ])
    self.skip_projection = Conv1dLayer(params.residual_channels, params.residual_channels, 1)
    self.output_projection = Conv1dLayer(params.residual_channels, params.in_channels, 1)
    nn.init.zeros_(self.output_projection.conv1d.weight)
    self.l_cond_dim = params.n_mels
    self.g_cond_dim = params.style_dim
